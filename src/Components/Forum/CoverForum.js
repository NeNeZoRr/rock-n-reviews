...
function CoverForum() {
	...
	const handleTopicSubmit = () => {
		if (newTopic.title.trim() && newTopic.body.trim()) {
			...
			setPostResult({
				success: false,
				message: "Please provide both title and body for the topic.",
			});
		}
	};
	...
	return (
		<div style={{ height: "100vh" }}>
			<h1 style={{ color: "white", textAlign: "center" }}>Forum</h1>
			{postResult.message && (
				<div
					style={{
						color: postResult.success ? "green" : "red",
						margin: "10px 0",
					}}>
					{postResult.message}
				</div>
								...
								<div>
									...
									<ReplyForm
										...
										onReplySubmit={(replyContent) =>
											handleReplySubmit(index, replyContent)
										}
									/>
								</div>
							</Accordion.Body>
