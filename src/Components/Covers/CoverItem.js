...
function CoverItem({ item }) {
  ...
  const detailView = () => (
    <div style={detailStyle}>
      <h2>
        Song Title: {view ? <Link to={`/song/${item.trackId}`}>{item.trackName}</Link> : item.trackName}
      </h2>
      <h3>Artist: {item.artistName}</h3>
      <h3>
        Album Title: {view ? <Link to={`/album/${item.collectionId}`}>{item.collectionName}</Link> : item.collectionName}
      </h3>
